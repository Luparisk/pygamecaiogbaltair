...
# ══════════════════════════════════════════════════════════════
CHARACTERS = [
    {"name":"Bruxinha","asset_key":"bruxinha","select_sprite":"characters/bruxinha/select.png",
     "speed":5,"max_hp":200,"shot_style":"potion",
     "shot_cd":14,"shot_speed":15,"shot_damage":8,"bullet_radius":7},
    {"name":"Vampiro","asset_key":"vampiro","select_sprite":"characters/vampiro/select.png"
     ,"speed":5,"max_hp":200,"shot_style":"triple",
     "shot_cd":24,"shot_speed":10,"shot_damage":6,"bullet_radius":5},
    {"name":"Dragaozinho","asset_key":"dragaozinho","select_sprite":"characters/dragaozinho/select.png"
     ,"speed":5,"max_hp":200,"shot_style":"fireball",
     "shot_cd":16,"shot_speed":0,"shot_damage":0,"bullet_radius":0,
     "max_charge_frames":60,"min_fire_speed":10,"max_fire_speed":20,
     "min_fire_damage":10,"max_fire_damage":30,"min_fire_radius":8,"max_fire_radius":18},
    {"name":"Esqueleto","asset_key":"esqueleto","select_sprite":"characters/esqueleto/select.png", 
     "speed":5,"max_hp":200,"shot_style":"boomerang",
     "shot_cd":22,"shot_speed":12,"shot_damage":7,"bullet_radius":5},
]
...
BOSS_VISUALS = {
    ...
    "clown": {
        ...
        "boss": SpriteAnim(load_folder_frames(ASSET_DIR / "bosses/clown/boss", (150, 200)), fps=8),
        "balloon": load_image(ASSET_DIR / "bosses/clown/balloon/balloon.png", (44, 44)),
    ...
    "frank": {
        ...
        "doctor": SpriteAnim(load_folder_frames(ASSET_DIR / "bosses/frank/doctor", (60, 100)), fps=8),
        "monster": SpriteAnim(load_folder_frames(ASSET_DIR / "bosses/frank/monster", (110, 160)), fps=8),
        "bullet": load_image(ASSET_DIR / "bosses/frank/bullet/bullet.png", (18, 18)),
...
class ClownBoss:
    ...
    def update_phase1(self, dt, player, vert_balloons, track_balloons):
        ...
        if self.vert_balloon_cd <= 0:
            vert_balloons.append(VerticalBalloon(sprite=BOSS_VISUALS["clown"].get("balloon")))
        ...
        if self.track_balloon_cd <= 0:
            track_balloons.append(
                TrackerBalloon(
                    ...
                    player.rect.centerx,
                    player.rect.centery,
                    sprite=BOSS_VISUALS["clown"].get("balloon")))
...
def draw_monster_bg(surf,tick,rage=False):
    ...
    for px in [140, BOSS_W - 140]:
        draw_detailed_pillar(surf, px, BOSS_H - 80, 220, pulse=tick, active=True)
...
class DrFrankenstein:
    ...
    def _shoot_one(self,player,speed=None,damage=None,spread=0.0):
        ...
        self.bullets.append(DrBullet(self.rect.centerx,self.rect.centery,math.cos(ang)*spd,math.sin(ang)*spd,damage=dmg, sprite=self.v.get("bullet")))
    ...
    def draw(self,surf):
        ...
        if img:
            center_blit(surf, img.get(self.bg_anim / 10.0), self.rect.center)
        for px in TESLA_XS:
...
class MonsterPhase:
    ...
    def draw(self, surf):
        img = self.v.get("monster")
        if img:
            center_blit(surf, img.get(self.bg_anim / 10.0), (BOSS_W // 2, BOSS_H // 2 - 20))
        ...
        if clown_phase == 1:
            ...
            for ball in boss.balls[:]:
                if ball.rect.colliderect(player.rect):
                    player.take_damage(18 if boss.rage else 14)
